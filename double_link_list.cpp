...
template <class Node_entry>
Node<Node_entry>::Node(Node_entry data, Node<Node_entry> *link_back = NULL, Node<Node_entry> *link_next = NULL)
{
	entry = data;
	back = link_back;
	next = link_next;
...
void List<List_entry> ::traverse(void(*visit)(List_entry &))
{
	...
	if (to_visit != NULL)
		for (; to_visit->back; to_visit->back)
...
void main()
{
	cout << "(操作)初始化一个linkedlist" << endl;
	List<int> linkedlist;
	cout << "初始时：" << endl;
	cout << "链表的长度：" << linkedlist.size() << endl;
	cout << "链表是否为空" << linkedlist.empty() << endl;
	cout << "链表是否为满" << linkedlist.full() << endl;

	cout << "（操作）向空链表插入 位置：值 0:0 1:1 2:2 3:3" << endl;
	...
	cout << "(操作)取出0，1，2的元素" << endl;
	int p0, p1, p2;
	linkedlist.retrieve(0, p0);
	linkedlist.retrieve(1, p1);
	linkedlist.retrieve(2, p2);
	cout << "取出的0，1，2位置元素分别为：" << p0 << "," << p1 << "," << p2 << endl;

	cout << "（操作）删除1位置的元素" << endl;
	int r1;
	linkedlist.remove(1, r1);
	cout << "删除1位置的元素后：" << endl;
	cout << "链表的长度：" << linkedlist.size() << endl;
	cout << "链表是否为空" << linkedlist.empty() << endl;
	cout << "链表是否为满" << linkedlist.full() << endl;
	cout << "删除的元素的值为：" << r1 << endl;;
	linkedlist.retrieve(0, p0);
	linkedlist.retrieve(1, p1);
	cout << "取出0，1位置的元素分别为：" << p0 << "," << p1;
